...
function preload() {
    yantraImg = loadImage('./img/luna2.jpg');
}
...
function setup() {
    ...
    // Ensure button listens for click events
    document.getElementById('saveKolamPattern').addEventListener('click', saveCanvasImage);
}

// Function to save the canvas as an image
function saveCanvasImage() {
    saveCanvas('kolam-pattern', 'png');
}
...
function mouseClicked() {
    ...
    // Define canvas boundaries with 100px margin
    let xMin = -100, xMax = width + 100;
    let yMin = -1, yMax = height + 1;

    // Check if the click is within the allowed area
    if (x < xMin || x > xMax || y < yMin || y > yMax) {
        return; // Ignore clicks outside the area
    }
    ...
    limit = movementValue;  // Update limit with the calculated movement value
    console.log("Updated Limit:", limit);  // Debugging the updated limit

    // Update the message based on the new tnumber and limit
    updateMessage();

    // If the limit exceeds 0.9, move to the next tnumber
...
function updateMessage() {
    const message = generateMessage(tnumber, limit);  // Generate the message based on tnumber and limit
    console.log("Message:", message);  // Debugging the generated message
    document.getElementById("messageBox").innerText = message;  // Display the message on the page
}

function generateMessage(tnumber, limit) {
    const tMessage = getTnumberMessage(tnumber);
    const lMessage = getLimitMessage(limit);
